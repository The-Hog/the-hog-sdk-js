...
import { TheHogDefaultError } from "../models/errors/the-hog-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | TheHogDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | TheHogDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new TheHogDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new TheHogDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
